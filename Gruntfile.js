...
module.exports = function (grunt) {  // load all grunt tasks
    ...
    grunt.initConfig({
        path: {  // configurable paths
            app: __dirname,
            build: __dirname,
            dist: 'img'
        },
        sprites: {  // build sprite image and sprite less-file
            sourcePath: '<%= path.build %>/test2/',
            webPath: '/',
            lessPath: '<%= path.build %>/test2/sprite.less'
        }
    });

    grunt.registerTask('sprite', 'Simple sprite generation', function() {

        var basePath = path.resolve(grunt.option('path') || grunt.config('sprites.sourcePath'));
        var done = this.async();
        ...
        sprite.sprites({path: basePath}, function(err, helper) {
            ...
            _.each(helper, function(pack){
                if (pack.name.indexOf('-2x')==-1) {
                    lessStr += "." + pack.name + "{\n";
                    lessStr += '\tbackground: url("/sprites' + grunt.config('sprites.webPath') + pack.filename() + '") no-repeat 0 0;\n';
                    ...
                    lessStr += "." + normalName + "{\n";
                    lessStr += '\tbackground: url("/sprites/' + grunt.config('sprites.webPath') + normalFileName + '") no-repeat 0 0;\n';
                    lessStr += '\tvertical-align: text-top;\n';
                    lessStr += '\tdisplay: inline-block;\n';
                    lessStr += "}\n";

                    lessStr += "." + pack.name + "{\n";
                    lessStr += '\tbackground: url("/sprites/' + grunt.config('sprites.webPath') + retinaFileName + '") no-repeat 0 0;\n';
            ...
            var lessPath = grunt.option('path') + 'sprite.less' || grunt.config('sprites.sourcePath');
            fs.writeFile(path.resolve(lessPath), lessStr, function(){
                if (err) {
                    throw err;
                }
                console.log('File ' + lessPath.cyan + ' was updated.');
                done(true);
            });
